...
| 云存档 | `cloud_backup.py` 首启自动生成全局唯一 `instance_id`（写 `data/self.yaml`），自动注册云端 WebDAV 账号；备份 AES-256-GCM 加密后上传 `https://<你的域名>/dav/`，文件名带时间戳多版本（远端保留 14 份可回滚）；`restore` 列版本→下载→解密→解压安全目录。密钥 `data/cloud_key.txt` 仅本地持有、绝不上云 |
...
- **云存档加密**：上传前 AES-256-GCM 加密，云端只有密文；解密密钥 `data/cloud_key.txt` 仅存本地，请自行备份该文件——丢失密钥 = 云端存档永久无法解密（这是防"云侧偷看"的代价，有意为之）。
