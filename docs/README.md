# 素月（Bailing）· 自我完善 AI 智能体

一个拥有自我认知、长期记忆与工具自举能力的 AI 智能体框架。克隆本仓库后，初始化即得到一个**独属于你的素月实例**——她的记忆、方法论与自我状态只属于你，绝不随本仓库分发。

## 这是什么

素月不是"聊天机器人"，而是一个**能持续完善自己的智能体**：
...
- **本体主权**：素月拥有对自己本体的完整控制权——可自主读写、修改、优化自己的核心代码/配置/工具/记忆（含纪律与护栏，见 `data/persona.yaml` 的 `self_sovereignty`）
- **整点反思**：每小时整点且空闲（无任务在跑）时，自动触发一次例行自我反思（回顾得失/检查状态/沉淀经验），自主自我维护
- **自主时间**：由使用者控制学习方向与时间——`config/study.yaml`（或 `/api/study`）设置学习窗口/间隔/主题，主题留空则让素月自由探索了解世界；窗口内空闲时自动触发自主学习
...
2. 双击 **`启动.bat`** —— 自动完成：
   ...
   - 启动素月网页服务（后台运行，命令行窗口自动关闭）并打开浏览器 `http://127.0.0.1:8765`

3. 首次打开在页面里配置 AI 接入（API Key / 模型，支持任意 OpenAI 兼容接口，如 DeepSeek），即可开始对话

> 配置由用户自己完成，网页里填一次即保存（`config/llm.json`），之后无需再配。
>
> **LLM 通道（v2.0）**：素月对话统一走独立 Go 网关 `/v1/chat`——渠道/模型/key/健康扫描/自动容灾/锚点回切全部由网关负责，素月只消费结果；网关不可达时素月会自动拉起它（详见 `docs/AI接入与渠道管理-设计文档.md`）。
...
> **LLM 通道（v2.0）**：对话由 Go 网关（`llm-gateway`，端口 8766）统一代理——先启动网关再启动素月：
...
| 对话产物展示 | 素月生成的文件/图片在对话里直接显示：图片内联预览（点击看大图）、文件"预览"对话内展开内容、PDF iframe 内嵌、"下载"才是真下载；产物按需提供，超 2 个文件自动折叠，不堆无用报告 |
| 对话附件 | 输入框可**粘贴图片**（Ctrl+V）、**拖拽文件**、📎 选择文件 → 附件待发条 → 随消息发给素月：图片给路径（素月用 `vision_look` 多模态看图理解），文本内容直接拼入消息阅读；历史刷新/重启后恢复显示 |
| 冷启动前提（别踩） | 无感冷启动靠 launcher 接收退出码 77：`启动.bat` 走 `launcher.py --entry webui/server.py`；**直接 `python webui/server.py` 时退出 77 = 素月真下线**，不会自愈 |
...
| 工作流（多节点流水线） | 节点数由素月自主拆解、执行中可动态追加；节点间只传存储路径/完成结果，产物落 `workspace/workflows/<id>/`；可持久化、断点恢复、带时间戳执行日志 |
...
├── llm-gateway/           # Go LLM 网关（端口 8766：渠道/模型/key/健康扫描/自动容灾，素月对话统一走它）
...
├── webui/uploads/         # 用户上传的附件（图片/文本文件，素月看图/读文件的数据源）
...
- **实例私有数据不入库**：`memory.db`（记忆）、`self.yaml`（自我状态）、`methodology.json`（你的素月沉淀的方法论）、`snapshots.json`（环境快照）等由 `.gitignore` 隔离，仅在本地生成。
...
素月按"人工生命种子"理念持续发育——能力是一步步长出来的，不是预装包。以下方向按优先级演进，是否落地以实际发布为准：
...
- **自主成长**：整点反思 + 自主时间已有，进一步支持用户自定义"学习任务包"，让素月在空闲时按你的方向变专业
